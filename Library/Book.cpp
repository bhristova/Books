...
Book::Book(const char* t, int pg, int yr, Author* a, const Genre& g, double pr): title(nullptr)
{
    setTitle(t);
    setPages(pg);
    setYear(yr);
...
bool Book::operator<(const Book& p) const
{
    return (strcmp(p.getTitle(),this->getTitle())<0 && this->getPages()<p.getPages() && this->getYear()<p.getYear()
            && !this->getAuthor()->areTheSame(p.getAuthor()) && this->getGenre()<p.getGenre() && this->getPrice()<p.getPrice());
}
